...
const MyOrders = () => {
  ...
  const url = `https://byke-app.herokuapp.com/user/${email}`;
  useEffect(() => {
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
  ...
  const deleteOrder = (id) => {
    ...
    if (confirmation) {
      const url = `https://byke-app.herokuapp.com/myOrders/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
    ...
    <div className="text-center my-5">
      {isLoading ? (
        ...
        <Container id="packages" className="mt-4">
          <h2 className="text-center text-success rounded-3 border-bottom pb-3 my-5 border-dark border-5 my-5">
            My Orders.
          </h2>
